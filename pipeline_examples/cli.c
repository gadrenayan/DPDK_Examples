	...
	size_t out_size)
{
	...
	gmempool = mempool_create(name, &p);
	printf("GNA: Mempool created: %p rte_mempool: %p\n", gmempool, gmempool->m);
	if (gmempool == NULL) {
	...
	size_t out_size)
{
	...
	printf("GNA: %s swq: %s ring: %p\n", __FUNCTION__, name, swq->r);
}
	...
	size_t out_size)
{
	...
	printf("GNA: %s Enabling thread: %d, %s\n", __FUNCTION__, thread_id, pipeline_name);
