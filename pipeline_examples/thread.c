...
struct thread_data tdata[RTE_MAX_LCORE];
...
thread_init(void)
{
	...
	RTE_LCORE_FOREACH_SLAVE(i) {
		...
		struct thread *t = &thread[i];
		struct thread_data *t_data = &tdata[i];
		...
		if (msgq_req == NULL) {
			printf("GNA: %s rte_ring_create FAILED: %s\n", __FUNCTION__, name);
			thread_free();
			return -1;
		}
		printf("GNA: %s msgq_req created: %s\n", __FUNCTION__, name);
	...
	const char *pipeline_name)
{
	...
	if ((thread_id >= RTE_MAX_LCORE) ||
		...
		(p->n_ports_out == 0) ||
		(p->n_tables == 0)) {
		printf("GNA: %s Failed name = %s, p = %p, in = %d, out = %d, tables = %d\n", __FUNCTION__, pipeline_name, p, p->n_ports_in, p->n_ports_out, p->n_tables);
		return -1;
	...
	if ((t->enabled == 0) ||
		p->enabled) {
		printf("GNA: %s Failed enabled = %d, p->enabled = %d\n", __FUNCTION__, t->enabled, p->enabled);
		return -1;
	}

	if (!thread_is_running(thread_id)) {
		struct thread_data *td = &tdata[thread_id];
...
thread_main(void *arg __rte_unused)
{
	...
	printf("GNA: %s Running tight loop threads: %d, %s\n", __FUNCTION__, t->n_pipelines, pname);
	/* Dispatch loop */
	for (i = 0; ; i++) {
		sleep(1);
		if (strcmp(pname, "INGRESS") == 0) {
			rte_pipeline_run_ingress(t->p[0]);
		} else if (strcmp(pname, "PASSTHROUGH") == 0) {
			rte_pipeline_run_passthrough(t->p[0]);
		} else if (strcmp(pname, "EGRESS") == 0) {
			rte_pipeline_run_egress(t->p[0]);
		}
